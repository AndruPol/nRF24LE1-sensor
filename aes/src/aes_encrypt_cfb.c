...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
// void aes_encrypt_cfb(aes_data_t * aes_data_p, const unsigned char * input_plain_text_block,
//                      unsigned char * output_cipher_text_block, unsigned int len)
//
// Description:
//  Encrypts an input block of plain text into an output block of cipher text using cipher feedback (CFB) mode
//
// Parameters:
...
void aes_encrypt_cfb(aes_data_t * aes_data_p, const unsigned char * input_plain_text_block, unsigned char * output_cipher_text_block, unsigned int len) __REENTRANT
{
	...
	for(i = 0; i < len; i++)
	{
		...
		// 2. Set the current output cipher text byte to the result of (1)
		// 3. Decrement the number of remaining key block bytes
		aes_data_p->iv_or_intermediate_data[16 - aes_data_p->rem_key_bytes] ^= input_plain_text_block[i];
		output_cipher_text_block[i] = aes_data_p->iv_or_intermediate_data[16 - aes_data_p->rem_key_bytes];
